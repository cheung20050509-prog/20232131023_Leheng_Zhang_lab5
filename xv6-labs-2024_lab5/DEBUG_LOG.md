...
## 5. 最终测试与 DNS 问题修复
- **测试**: 运行 `make grade`。
- **现象**: `dns` 测试失败，提示 `invalid DNS reply` 或超时。
- **排查**: 通过在 `nettest.c` 中增加 `printf` 调试，发现接收到的 UDP 数据长度计算有误，切掉了部分 payload。
- **原因**: `e1000_recv` 中传递给 `net_rx` 的长度应该是网络包的实际有效长度，但在 `sys_recv` 中 `copyout` 时未正确处理 `udp->ulen`。
- **修正**: 确保 `len` 计算逻辑严谨 `ntohs(udp->ulen) - sizeof(struct udp)`。
- **结果**: `make grade` 全部通过，得分 171/171。

## 6. 总结
本次实验主要难点在于理解软硬件协同工作的 Ring Buffer 机制以及并发环境下的锁管理。通过逐步调试和日志分析，解决了内存泄漏和并发竞争问题。
